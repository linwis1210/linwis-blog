...
Use the source-of-truth index in `AGENTS.md` for actual project paths. This project's established sources of truth are:

```text
/
├── AGENTS.md
├── DESIGN.md                 # design system; referenced by src/config/effects.ts
├── docs/
│   ├── REQUIREMENTS.md       # product requirements
│   ├── ARCHITECTURE.md       # architecture and technical constraints
│   └── TASKS.md              # long-term planning and task backlog
└── .agents/
    ├── roles/
    │   ├── leader.md
    │   ├── builder.md
    │   └── verifier.md
    └── skills/               # project-local skills (e.g. frontend-design)
```

Application source lives under `src/` (Astro: `components/`, `config/`, `content/`, `layouts/`, `lib/`, `pages/`, `styles/`); discover the current structure from the repository rather than assuming a fixed layout.
